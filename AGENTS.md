...
| Chat thread folders and proposals; typed judgment | `thread-folders.md`, `typed-judgment.md` |
...
- Do **not** silently modify requirements to match an implementation; propose divergence explicitly.
  An architectural conflict with the plan requires a **proposed ADR** in `docs/adr/` (index at `docs/adr/index.md`).
...
- Milestones 0 through 12, 16, and 17 are complete. Milestones 13 through 15 proceed in that order (ADR-0061);
  18 through 29 are parallel workstreams (ADRs 0071–0073, 0077, 0079–0082, 0092, 0097, 0100, 0102). Milestone 21 is limited to adaptive
  memory distillation, 22 to the persona surface, 23 to the conversational schedule lifecycle, 24 to SMS through
  the owner's iPhone, 25 to the WhatsApp business surface, 26 to the approved client modes and adaptive email experience, 27 to Bland calling, 28 to People and relationship memory, and 29 to chat thread folders and typed judgment. Model routing and the plan's remaining roadmap items
  are not authorized. Open items by milestone: `docs/status/milestones.md`.
