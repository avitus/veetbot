...
effort (ADR-0119) failed development precision, the rich conversation core,
and holdout direct recall and precision, so Astra at medium has no artifact
either.

Since ADR-0119 a `formation@9` artifact also binds the reasoning effort its
evaluated arm sent: schema 8 records it, and a schema 7 artifact, like both
Sol artifacts below, is provider-default evidence that matches only a null
effort. Every other bundled `formation@9` tuple on this tree is a memory
choice the owner may select from the client.
