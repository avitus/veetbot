...
class TrajectoryExportService:
    ...
    async def export(self, run_id: UUID) -> ArtifactRef:
        ...
        async with self._uow_factory() as uow:
            ...
            if existing is not None:
                ...
                if existing.artifact.expires_at is not None and existing.artifact.expires_at > now:
    ...
    async def read(self, run_id: UUID) -> bytes:
        ...
        if (
            export is None
            or export.artifact.expires_at is None
            or export.artifact.expires_at <= self._clock.now()
        ):
