...
async def test_post_transition_prologue_failure_reaches_terminal_state(
    ...
    async def fail_seed(_executor: RunExecutor, _run: object) -> object:
        raise RuntimeError("synthetic checkpoint seed failure")

    monkeypatch.setattr(RunExecutor, "_seed_checkpoint", fail_seed)
    async with build(
        settings=_development_settings(),
        clock=FixedClock(NOW),
        ids=SequenceIdFactory(),
