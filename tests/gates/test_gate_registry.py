...
def test_registry_bound_follows_the_authorized_milestones(tmp_path: Path) -> None:
    """Milestone 28 is authorized; the registry admits it and stops there."""
    import scripts.gate_registry as gate_registry

    assert getattr(gate_registry, "MAX_MILESTONE", None) == 28
    ...
    (plan_dir / "milestone-map.md").write_text(
        "## The gate table\n\n```text\n"
        "gate.schedule.roadmap_probe   case   28\n"
        "gate.schedule.beyond_probe    case   29\n"
        "```\n\n## The census\n\n```text\n```\n",
        encoding="utf-8",
    )
    ...
    (gates / "schedule.yaml").write_text(
        yaml.safe_dump([entry("roadmap_probe", 28), entry("beyond_probe", 29)]),
        encoding="utf-8",
    )
    errors = registry_errors(tmp_path)
    assert "gate.schedule.roadmap_probe has invalid milestone 28" not in errors
    assert "gate.schedule.beyond_probe has invalid milestone 29" in errors
