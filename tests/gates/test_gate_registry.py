...
def test_gate_table_arithmetic_reports_stale_digits() -> None:
    ...
    assert gate_table_arithmetic_errors(stale, _GATE_TABLE_DERIVED) == [
        "gate table intro says 15 subject specs; registry derives 28",
        "gate table intro says 178 subject gates; registry derives 427",
        "gate table intro says 187 declarations; registry derives 436",
        "gate table intro says 184 entries; registry derives 433",
    ]
...
def test_gate_table_arithmetic_rejects_compensating_component_edits() -> None:
    shifted = (
        ...
        "declares 3 more, and this document declares 6 over the corpus: 436\n"
...
def test_registry_bound_follows_the_authorized_milestones(tmp_path: Path) -> None:
    """Milestone 29 is authorized; the registry admits it and stops there."""
    import scripts.gate_registry as gate_registry

    assert getattr(gate_registry, "MAX_MILESTONE", None) == 29
    ...
    (plan_dir / "milestone-map.md").write_text(
        "## The gate table\n\n```text\n"
        "gate.schedule.roadmap_probe   case   29\n"
        "gate.schedule.beyond_probe    case   30\n"
        "```\n\n## The census\n\n```text\n```\n",
        encoding="utf-8",
    )
    ...
    (gates / "schedule.yaml").write_text(
        yaml.safe_dump([entry("roadmap_probe", 29), entry("beyond_probe", 30)]),
        encoding="utf-8",
    )
    errors = registry_errors(tmp_path)
    assert "gate.schedule.roadmap_probe has invalid milestone 29" not in errors
    assert "gate.schedule.beyond_probe has invalid milestone 30" in errors
...
def test_schedule_lifecycle_has_complete_milestone_23_gate_area() -> None:
    entries, errors = load_registry(ROOT)
    assert errors == []
...
def test_device_channel_has_complete_milestone_24_gate_area() -> None:
    entries, errors = load_registry(ROOT)
    assert errors == []
    device_entries = [entry for entry in entries if entry.milestone == 24]
...
def test_whatsapp_surface_has_complete_milestone_25_gate_area() -> None:
    entries, errors = load_registry(ROOT)
    assert errors == []
    whatsapp_entries = [entry for entry in entries if entry.milestone == 25]
...
def test_email_experience_has_complete_milestone_26_gate_area() -> None:
    entries, errors = load_registry(ROOT)
    assert errors == []
    email_entries = [entry for entry in entries if entry.milestone == 26]
    assert len(email_entries) == 32
    assert all(entry.id.startswith("gate.email.experience_") for entry in email_entries)
    assert all(entry.spec == "docs/plan/email-experience.md#hard-gates" for entry in email_entries)
    assert all(GATE_ID.fullmatch(entry.id) for entry in email_entries)


def test_bland_calling_has_fourteen_registered_milestone_27_gates() -> None:
    entries, errors = load_registry(ROOT)
    assert errors == []
    calls = [entry for entry in entries if entry.milestone == 27]
    assert len(calls) == 14
    assert all(entry.id.startswith("gate.call.") for entry in calls)
    assert all(entry.spec == "docs/plan/bland-calling.md#hard-gates" for entry in calls)
