...
async def test_browser_authentication_control_plane_contract(tmp_path: Path) -> None:
    ...
    await service_contract.assert_authentication_ceremony_is_direct_single_use_and_runtime_decided(
        tmp_path / "needs-user",
        runtime_status=BrowserAuthenticationStatus.NEEDS_USER,
    )
    cancel_contract = (
        service_contract.assert_authentication_cancellation_is_scoped_idempotent_and_closes_runtime
    )
    await cancel_contract(tmp_path / "cancel")
