...
def test_docs_checks_admit_the_roadmap_milestones(
    ...
    """Milestones 12 through 17 are authorized; project state and plan checks follow."""
    monkeypatch.syspath_prepend(str(ROOT / "scripts"))
    check_docs = importlib.import_module("check_docs")

    status = tmp_path / "docs" / "status"
    status.mkdir(parents=True)
    milestones = {str(n): {"title": f"milestone {n}", "status": "planned"} for n in range(18)}
