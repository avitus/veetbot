"""Schedule wakeup connections are reused and recover after failure."""
...
async def test_postgres_wakeup_reuses_one_listener_connection(
    ...
    monkeypatch.setattr("agent_core.adapters.schedule_wakeup.asyncpg.connect", connect)
    wakeup = PostgresScheduleWakeup("postgresql://localhost/test")
...
async def test_postgres_wakeup_drops_failed_publisher(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert wakeup._publisher is None
