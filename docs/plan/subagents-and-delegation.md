...
(context-engine.md:359-362, memory-retrieval-and-ranking.md:96); and the
...
seeds from its brief and from nothing else (context-engine.md:359-362), so
...
7. Inserts one `delegations` row carrying every child and marks the parent's
   ...
   tool-system.md:969-972 already declares). Steps 3 through 6 repeat per
...
authorizations (tool-system.md:977-983). A child result that instructs the
...
rule tool-system.md:975-979 already states for a suspended invocation whose run
