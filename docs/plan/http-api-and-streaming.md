...
baseline or its gate census. ADR-0053 adds the principal-scoped session-message
...
`daily_spent`, `daily_reserved`, `daily_limit`, `monthly_spent`,
`monthly_reserved`, `monthly_limit`, and `next_reservation`, plus an ISO-8601 `retry_at` for the next
admission check. It is not a promise of replenishment: new spending or unresolved
provider accounting may keep admission closed. Other budget failures retain
their existing empty details. These responses remain private and non-cacheable.
...
client-supplied reserved key or either bound violation is `malformed_request`.
...
ADR-0053 adds the read counterpart to the existing message-submission path:
...
Section 21 already says. The two ADR-0050 routes and ADR-0053 transcript route
...
13 current history routes           session.read/write; ADR-0053 makes surface 17
```
...
4. Should `GET /v1/runs` exist — a list of runs in a session? ADR-0053 shows
...
foreign session is `404`, and a closed one is `409` as a message to it would be. A new
