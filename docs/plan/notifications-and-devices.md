...
8. **Exactly the five transitions enqueue, and one named producer besides.**
   ...
   Milestone 24 widens the catalog to six with the pending device invocation;
   the sixth trigger's behavior is gated by that milestone (ADR-0081).
...
If that deadline passes while the receipt is being committed, the call and
receipt still persist but no expired notification is enqueued. Creation and
next-attempt timestamps share one clock reading before the retention deadline.
...
presentation memory and clears it on logout or connection replacement. Only the
