...
**The loop cannot resolve its own agent.** `engineering-plan.md:1525` reads
...
**The loop cannot suspend.** `engineering-plan.md:1550` handles a paused
...
`finally`. A run that pauses for approval under the pseudocode as written
holds its lease until the lease expires, at which point the queue reclaims
it and a second worker starts executing a run that is waiting for a human.
