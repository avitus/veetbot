...
claim batch, lease, fallback poll, retry schedule, and terminal expiry, plus
Milestone 16's memory lifecycle knobs — five decay time constants, two ranking
...
selection is independently configured by the policy-name reference
`formation.model_policy` in `memory/profiles.yaml`; this selection is included in
...
knobs — decay time constants, ranking penalties, usage deltas, the decay sweep's
...
class Settings:
    ...
    schedule_api_enabled: bool           # deployment feature flag, default false
    schedule_worker_enabled: bool        # deployment feature flag, default false
```
...
that evidence artifact, plus the two schedule feature flags above; these are
deployment controls rather than tuning knobs and therefore remain in
`Settings`. `credentials` is keyed by provider profile name rather than by a
...
Twelve canonical names for ten fields, which is not a discrepancy.
...
The remaining eight names map one-to-one onto the eight scalar fields.
...
within the ordinary preparation slots, and closes it. Serving does not wait for
it, and one-shot CLI commands and tests never start it.
...
on that basis, making the reserved set four words. ADR-0131 adds `latency`,
a read-only, aggregates-only latency report over recent Chat turns, making
it five. A spec adding one pays exactly two costs — a line here and one more
prompt that needs `--` — and both are cheaper than a thirteenth top-level
noun, because the twelve is a number Section 17 states and this document's
own heading repeats.
