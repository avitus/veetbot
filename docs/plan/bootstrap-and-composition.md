...
The corpus now declares **184 configuration knobs** across the specifications;
...
period, and ADR-0123 adds the deferred tool index's item and token caps. The plan names **three
...
1.  **Shipped defaults.** YAML committed inside the package, next to the
    module that owns it. This is where all 184 knobs live, at the values the
    specs state.
...
and asserts the total is 184. Schema versions, profile names, rule identifiers,
...
2.  **`.env.example` versus 184 file-layer knobs.** The definition of done
...
2. **A value is an environment variable if and only if it differs between
   ...
   sorts all 184 declared knobs into files and leaves ten fields in
   `Settings`.
