...
could already exist at `engineering-plan.md:452` with no design behind
...
`tool-system.md:1315` says *"`skill.load` and `skill_manage` are
...
`tool-system.md:1317` requires the `skills:write` scope. That string
...
4  "skills have no design at all"     tool-system.md:1274-1321
...
second half is true. The first is not: `tool-system.md:1274-1321` is
...
3. Should there be a catalog surface — `GET /v1/skills`, or an
   ...
   route table is thirteen routes and the CLI is twelve commands,
