...
could already exist at `engineering-plan.md:496` with no design behind
...
Section 30.2 at `engineering-plan.md:4008` calls it *"a skill_manage
...
sentence was wrong, and `tool-system.md:1349` carries the corrected
sentence now.
...
`tool-system.md:1353` requires `skill.write`. The earlier string was wrong in
two ways against
...
4  "skills have no design at all"     tool-system.md:1308-1355
...
second half is true. The first is not: `tool-system.md:1308-1355` is
...
`readiness.md:739-741` cited `engineering-plan.md:2791` for the
version-pinning acceptance criterion, which is at
`engineering-plan.md:2791`; the line it named is an MCP
