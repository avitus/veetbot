...
and appends exactly one event, which must commit (`runtime-loop.md:868-877`).
...
alongside the run insert (`http-api-and-streaming.md:895`), while a worker may
...
converter ever sees it (`evaluation-harness.md:1328`). This is the
...
input (`sandbox-isolation.md:1113`), an authorized read path that ADR-0028
...
(`sandbox-isolation.md:1061`). The origin matters because it is the one
...
discards timestamps at conversion (`evaluation-harness.md:1305`) and a
...
2. **Pattern replacement.** The secret scanner's five rule families
   (`bootstrap-and-composition.md:1235-1243`) run over every message body,
...
(`bootstrap-and-composition.md:1248-1250`).
...
artifact retention this corpus already sets (`sandbox-isolation.md:1177`).
...
(`bootstrap-and-composition.md:1070`), which is cheaper than a thirteenth
