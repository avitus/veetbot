...
`mark_effect_sent` implements (tool-system.md:659-662) — so `effect_sent_at`
...
`UNCERTAIN` (tool-system.md:674). A rate limit, a 5xx, or a lost response
...
`tool.outcome_unknown` and `retryable: false` (tool-system.md:815-819), and is
...
(browser-automation.md:1001-1005), generalized from those two cases to every
