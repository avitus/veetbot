...
(tool-system.md:1005-1007): a stdio child inherits the worker's network
...
`mark_effect_sent` implements (tool-system.md:650-654) — so `effect_sent_at`
...
`UNCERTAIN` (tool-system.md:665). A rate limit, a 5xx, or a lost response
...
`tool.outcome_unknown` and `retryable: false` (tool-system.md:804-808), and is
blocked from being proposed again in the run by the unified breaker's
threshold-of-one row (tool-system.md:847). This is the rule
