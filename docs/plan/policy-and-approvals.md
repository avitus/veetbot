...
executable vocabulary twenty-five strings. Future authorized work in Milestone
...
[inbound-surfaces.md](inbound-surfaces.md); until then the twenty-five-string
count above stands.

### The grammar, and the contributor a closed list cannot hold

A scope is two or more lowercase segments matching `[a-z][a-z0-9_]*`
joined by dots, of which the last is the action. All twenty-five have
exactly two.
...
when it is one of the twenty-five, or when its first segment is `mcp` and its
...
means: all twenty-five, and no `mcp.` scope. Those exist only once a server
...
11. **Scope grammar.** Every entry in the twenty-five-string vocabulary and
...
31. The scope vocabulary is one closed set of twenty-five dotted strings through
    Milestone 12, shared by the API's route checks and by this pipeline's tool
    check. Milestones 11 and 12 add exact schedule, device, and notification
    application scopes without changing the Milestone 4 baseline gate.
