...
3.  **A reserved seam is not an omission** where the document says so.
    `tool-system.md:2101` states that device tools are *"a reserved
...
(Section 6.5)"* in Milestone 2 at `engineering-plan.md:2642`, while
...
1.  **Provider response metadata is designed nowhere.**
    ...
    a `dict[str, Any]` field at `engineering-plan.md:1335`. No document
...
only at `engineering-plan.md:585`, the policy spec identified where
...
`bootstrap-and-composition.md:649` named `ApprovalService` as one of
...
`engineering-plan.md:1931-2138`, designs the API more thoroughly than
...
3.  **`Idempotency-Key` handling is named in two places and specified
    ...
    `engineering-plan.md:2023` and as an implement bullet, and the
...
Section 28 of the plan is not empty — `engineering-plan.md:4556-4633`
...
existed. `tool-system.md:1041` constrains MCP server URLs by *"the
...
specified at `tool-system.md:736`, and the programmatic orchestration
bridge Section 8.5 requires is specified from `tool-system.md:1474`.

Two further items deserved naming.

1.  **The plan demands a red-team test with no case behind it.**
    `engineering-plan.md:4631` requires a container-escape attempt as
...
2.  **`sandbox.run_command` was placed at two milestones.**
    `builtin-tools.md` said Milestone 5 where `builtin-tools.md:1518`
...
remaining. The egress allowlist `tool-system.md:1041` depends on by
...
had to be written. `tool-system.md:1405-1452` draws the line between
...
`engineering-plan.md:2888` had no design behind it — and no document
...
`context-engine.md:400`, where `runs.seed_event_sequence` is nullable
...
restricted tool set is `tool-system.md:1008`: *"the registry resolves
...
`tool-system.md:1004`. Two are partial: the explicit objective has a
carrier but no schema, since `delegate.run` is a control tool at
`tool-system.md:945` and no input type for it exists anywhere, and
the child budget is additive by `engineering-plan.md:679` while no
rule derives a child's own `limits`. Two still have none — the
separate trace and the artifact references, stated at
`engineering-plan.md:3289` and `engineering-plan.md:3290` and picked
...
number. A checked-in multi-session corpus of labeled beliefs and probes across
eight categories drives real formation and real retrieval through the
composition root on the in-memory tier under a fixed clock, a scripted model,
and sequential identifiers, so two runs of a commit agree byte for byte; the
metrics are integers, the checked-in baseline is compared exactly rather than
within a tolerance, and a behavior change therefore cannot land without
re-recording the baseline in the same change where its delta is reviewable. An
opt-in live arm pairs a with-memory and a without-memory run over the same
probes, scores them with checked-in labels, token-bounded matching, and one
exact abstention phrase rather than with a model judge, derives every threshold
from counts inside its own artifact, stops before crossing a pre-admission
ceiling of USD 4.00, and publishes evidence that re-validates itself and is
never overwritten. Three public long-horizon datasets are read from a local
path under their own licenses and never vendored, contributing an
evidence-provenance recall metric that distinguishes recalling the right fact
from recalling it for the right reason. The document then implements the
lifecycle the two Milestone 9 memory specifications describe and the code never
...
rather than new runtime. Seventeen hard gates live in the `email` area after
ADR-0085 adds four for operator-managed multi-account composition;
...
ADR-0085 extends the authorized scope to one or more operator-managed Gmail
accounts for the configured principal. The existing credential triplet remains
the backward-compatible single-account configuration; a bounded versioned
manifest composes three separately credentialed servers per account, retains
the legacy ids for the default account, and makes every other account explicit
in the tool name. Public self-service OAuth and per-principal token storage
remain undesigned and out of scope.
...
governed way to reach the existing application service. It accepts a title,
instruction, and exact aware instant, creates only `once` cadence, and is
registered only when both existing scheduling flags are on.

The tool is `EXTERNAL_WRITE`, high risk, conditionally idempotent, requires
exactly `schedule.write`, and always waits for approval under the existing
policy matrix. It delegates no scopes to the future run, applies finite
per-occurrence limits, routes through `ScheduleService.create`, and preserves
the existing idempotency key. Past or naive instants fail validation, and a
retry returns the original schedule rather than creating a duplicate. The
notification path is unchanged: the scheduled run reaches an ordinary
...
`formation@9`, `retrieval@3`, and `lifecycle@2` are new versioned behavior, and
the repaired provider-assisted `formation@10` of ADR-0086, with the compiled
policy binding of every bundled artifact, is designed in the same document.
...
durable corrections still form nothing. ADR-0090 adds a typed communication
exception after a production Gmail run formed zero memories: repository-owned,
principal-scoped Gmail and SMS events may form only channel-attributed,
tentative, local, sensitive hypotheses, while paired-surface owner messages use
the existing owner-assertion lane and arbitrary external content remains barred.
Ambiguity, inference, ongoing state, and sensitivity permitted by policy instead
affect derivation, confidence, longevity, expiry, and viewing ceilings. This is
the specific distinction the prior designs did not make strongly enough.

The readiness verdict is **Authorized**: ADR-0077 and ADR-0090, thirty-one new
...
no scheduling tool with which to resume it. [scheduling.md](scheduling.md) and
ADR-0080 add the missing model boundary without changing recurrence,
persistence, materialization, or the HTTP control plane.

On 2026-09-04 the owner expanded this workstream to conversational
modification. ADR-0088 adds `schedule.update`: a closed title, instruction,
and cadence patch whose stable identity, expected revision, exact
`schedule.write` authority, approval, immutable revision, and idempotent
recovery are fixed before implementation. The application service, rather
than the model, carries forward every hidden execution-authority and limit
field.

The model first uses bounded, summary-only `schedule.list` discovery to obtain
the stable ID and current revision for one principal-owned schedule. It then
uses approval-gated `schedule.pause`, `schedule.resume`, or `schedule.cancel`
with only that ID and `expected_revision`. Titles are never mutation keys;
ambiguity asks the owner. Existing exact scopes, optimistic concurrency,
no-backfill resume, state-idempotent lifecycle methods, and schedule/run
...
## Milestone 24: SMS through the owner's iPhone, authorized and specified
...
`gate.device.*` entries, and the Milestone 24 census row specify the
whole surface. Presence-based routing, hand-off, automatic replies, and
any silent send path remain outside it.

## Milestone 25: WhatsApp business surface, authorized and specified
...
The readiness verdict is **Authorized**: ADR-0082, the twelve new
`gate.whatsapp.*` entries, and the Milestone 25 census row specify the
...
`engineering-plan.md:4635-4792` for links to other documents returned
...
`tool-system.md:1557` does open a *"Device-scoped tools"* section, and
`tool-system.md:2101` states that device tools are *"a reserved seam,
...
one of them the question `tool-system.md:1571` reserved by name —
...
`engineering-plan.md:2888`, and the line this review first named was
...
4.  **The container-escape test and the case table.**
    `engineering-plan.md:4631` requires a test the harness's case set
...
2.  **Does the sandbox specification precede Milestone 5, or follow
    ...
    come first: `tool-system.md:1041` already depends on an egress
...
The owner approved the complete proposal, USD 20/day and USD 200/rolling
thirty-day aggregate automatic-email budgets, and implementation as a new
...
external verification requirements rather than design omissions.
...
recovery, the closed `mailto` path, fixed label actions, the deterministic
census, durable owner decisions, honest outcomes, flagged and scoped routes,
confined Chat tools, privacy under adversarial mail, persistence parity, the
