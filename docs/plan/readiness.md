...
3.  **A reserved seam is not an omission** where the document says so.
    `tool-system.md:2117` states that device tools are *"a reserved
...
2.  **Alembic had no authoring conventions.**
    `engineering-plan.md:1761` says *"Create Alembic migrations for at
...
(Section 6.5)"* in Milestone 2 at `engineering-plan.md:2655`, while
...
1.  **Provider response metadata is designed nowhere.**
    ...
    a `dict[str, Any]` field at `engineering-plan.md:1339`. No document
...
`bootstrap-and-composition.md:650` named `ApprovalService` as one of
...
`engineering-plan.md:1935-2151`, designs the API more thoroughly than
...
3.  **`Idempotency-Key` handling is named in two places and specified
    ...
    `engineering-plan.md:2027` and as an implement bullet, and the
...
existed. `tool-system.md:1057` constrains MCP server URLs by *"the
...
bridge Section 8.5 requires is specified from `tool-system.md:1490`.
...
remaining. The egress allowlist `tool-system.md:1057` depends on by
...
had to be written. `tool-system.md:1421-1468` draws the line between
...
`engineering-plan.md:2901` had no design behind it — and no document
...
`context-engine.md:446`, where `runs.seed_event_sequence` is nullable
...
restricted tool set is `tool-system.md:1024`: *"the registry resolves
...
`tool-system.md:1020`. Two are partial: the explicit objective has a
carrier but no schema, since `delegate.run` is a control tool at
`tool-system.md:961` and no input type for it exists anywhere, and
...
`engineering-plan.md:3302` and `engineering-plan.md:3303` and picked
...
`tool-system.md:1573` does open a *"Device-scoped tools"* section, and
`tool-system.md:2117` states that device tools are *"a reserved seam,
...
one of them the question `tool-system.md:1587` reserved by name —
...
`engineering-plan.md:2901`, and the line this review first named was
...
2.  **Does the sandbox specification precede Milestone 5, or follow
    ...
    come first: `tool-system.md:1057` already depends on an egress
