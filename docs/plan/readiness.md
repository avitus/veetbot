...
3.  **A reserved seam is not an omission** where the document says so.
    `tool-system.md:2065` states that device tools are *"a reserved
...
existed. `tool-system.md:1023` constrains MCP server URLs by *"the
...
specified at `tool-system.md:735`, and the programmatic orchestration
bridge Section 8.5 requires is specified from `tool-system.md:1440`.
...
2.  **`sandbox.run_command` was placed at two milestones.**
    `builtin-tools.md` said Milestone 5 where `builtin-tools.md:1517`
...
remaining. The egress allowlist `tool-system.md:1023` depends on by
...
had to be written. `tool-system.md:1371-1418` draws the line between
...
`context-engine.md:369`, where `runs.seed_event_sequence` is nullable
...
restricted tool set is `tool-system.md:990`: *"the registry resolves
...
`tool-system.md:986`. Two are partial: the explicit objective has a
carrier but no schema, since `delegate.run` is a control tool at
`tool-system.md:944` and no input type for it exists anywhere, and
...
`tool-system.md:1523` does open a *"Device-scoped tools"* section, and
`tool-system.md:2065` states that device tools are *"a reserved seam,
...
one of them the question `tool-system.md:1537` reserved by name —
...
1.  **`sandbox.run_command` at Milestone 5 or Milestone 6.** The
    ...
    HTTP API. `builtin-tools.md:1517` now says Milestone 6.
...
2.  **Does the sandbox specification precede Milestone 5, or follow
    ...
    come first: `tool-system.md:1023` already depends on an egress
