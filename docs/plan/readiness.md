...
3.  **A reserved seam is not an omission** where the document says so.
    `tool-system.md:2017` states that device tools are *"a reserved
...
Section 28 of the plan is not empty — `engineering-plan.md:4330-4407`
...
existed. `tool-system.md:1015` constrains MCP server URLs by *"the
...
bridge Section 8.5 requires is specified from `tool-system.md:1392`.

Two further items deserved naming.

1.  **The plan demands a red-team test with no case behind it.**
    `engineering-plan.md:4409` requires a container-escape attempt as
...
remaining. The egress allowlist `tool-system.md:1015` depends on by
...
had to be written. `tool-system.md:1323-1370` draws the line between
...
restricted tool set is `tool-system.md:982`: *"the registry resolves
...
`tool-system.md:978`. Two are partial: the explicit objective has a
carrier but no schema, since `delegate.run` is a control tool at
`tool-system.md:936` and no input type for it exists anywhere, and
...
`engineering-plan.md:4409-4567` for links to other documents returned
...
`tool-system.md:1475` does open a *"Device-scoped tools"* section, and
`tool-system.md:2017` states that device tools are *"a reserved seam,
...
one of them the question `tool-system.md:1489` reserved by name —
...
2.  **Usage token classes and cost-source precedence at Milestone 2 or
    ...
    `model-gateway.md:1810` and `milestone-map.md:1669`. The map
...
4.  **The container-escape test and the case table.**
    `engineering-plan.md:4409` requires a test the harness's case set
...
2.  **Does the sandbox specification precede Milestone 5, or follow
    ...
    come first: `tool-system.md:1015` already depends on an egress
