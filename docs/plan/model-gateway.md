...
This is not the `ModelCapabilities` of `engineering-plan.md:716`. Two
...
registry described at `engineering-plan.md:1405-1410`. The registry is
...
Section 10 (`engineering-plan.md:1389`) requires a run to be pinned to one
provider. Milestone 10 (`engineering-plan.md:3037-3044`) wants routing to move work
...
`engineering-plan.md:1405` calls a provider profile "a plugin the registry
loads and the user can override without editing core" and
`engineering-plan.md:1407-1410` says what it declares: an API mode, aliases
...
profile. `bootstrap-and-composition.md:385-386` places `models/policies.yaml`
...
the four things `bootstrap-and-composition.md:386` names it for and becomes
...
family of the secret scanner at `bootstrap-and-composition.md:1171-1212` is
...
`policy_version` at `policy-and-approvals.md:728` because it answers the
...
`engineering-plan.md:716` declares `ModelCapabilities` with eight fields and
...
```text
engineering-plan.md:716   here                     what changed
------------------------  -----------------------  ------------
...
Where the row for a field and `engineering-plan.md:716` conflict the plan
...
table to put them in. `runs.usage JSONB` at `engineering-plan.md:1797` is the
...
never disagree. The usage repository port named at `engineering-plan.md:927`
and never typed is:
...
`ModelTurn.provider_metadata` is declared at `engineering-plan.md:1322` as
`dict[str, Any]` and given exactly one rule at `engineering-plan.md:1324`:
...
| `response_id` | the response body | `engineering-plan.md:1367` requires the OpenAI adapter to capture it |
...
reads it, which is what `engineering-plan.md:1324`'s "application logic must
not rely on provider-specific fields" means once it is a rule a test can
evaluate.

## Retries, and who owns them

`engineering-plan.md:1370` puts retries in the adapter.
`engineering-plan.md:1697` says "Keep retry decisions in application code, not
...
`engineering-plan.md:954` and has no counterpart here, which means today a
...
provider-opaque items at `engineering-plan.md:711` — store verbatim, never
...
`engineering-plan.md:709` defaults `ProviderReasoningItem.trust_level` to
`TrustLevel.PLATFORM`. That is the highest trust tier in the system, and
`policy-and-approvals.md:923-952` maps trust tiers to policy restrictiveness,
...
Section 19's telemetry attributes (`engineering-plan.md:2247-2265`) omit the
...
Section 2.3's provider list at `engineering-plan.md:264-268` is controlling
...
(`engineering-plan.md:2671`) requires "the same contract suite against OpenAI,
Anthropic, and a chat_completions endpoint", while `engineering-plan.md:2415`
...
`engineering-plan.md:1330-1342` uses `FakeModelScript`, `ToolCallTurn` and
...
7. Is one file per provider profile right, given that
   `bootstrap-and-composition.md:385` describes a single `models/policies.yaml`
