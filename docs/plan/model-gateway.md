...
(`context-engine.md:917-919`). The gateway translates those hints into
...
The cached-prefix ratio is defined in `context-engine.md:895-897` and the
...
profile. `bootstrap-and-composition.md:397-398` places `models/policies.yaml`
...
the four things `bootstrap-and-composition.md:398` names it for and becomes
...
family of the secret scanner at `bootstrap-and-composition.md:1184-1225` is
...
`policy_version` at `policy-and-approvals.md:729` because it answers the
...
`policy-and-approvals.md:924-953` maps trust tiers to policy restrictiveness,
...
be answerable by a tool result; `context-engine.md:466-470` requires that a
...
7. Is one file per provider profile right, given that
   `bootstrap-and-composition.md:397` describes a single `models/policies.yaml`
