...
This is not the `ModelCapabilities` of `engineering-plan.md:670`. Two
...
registry described at `engineering-plan.md:1359-1364`. The registry is
...
Section 10 (`engineering-plan.md:1357`) requires a run to be pinned to one
provider. Milestone 10 (`engineering-plan.md:2988-2995`) wants routing to move work
...
`engineering-plan.md:1359` calls a provider profile "a plugin the registry
loads and the user can override without editing core" and
`engineering-plan.md:1361-1364` says what it declares: an API mode, aliases
...
`engineering-plan.md:670` declares `ModelCapabilities` with eight fields and
...
```text
engineering-plan.md:670   here                     what changed
------------------------  -----------------------  ------------
...
Where the row for a field and `engineering-plan.md:670` conflict the plan
...
table to put them in. `runs.usage JSONB` at `engineering-plan.md:1751` is the
...
never disagree. The usage repository port named at `engineering-plan.md:881`
and never typed is:
...
`ModelTurn.provider_metadata` is declared at `engineering-plan.md:1276` as
`dict[str, Any]` and given exactly one rule at `engineering-plan.md:1278`:
...
| `response_id` | the response body | `engineering-plan.md:1321` requires the OpenAI adapter to capture it |
...
reads it, which is what `engineering-plan.md:1278`'s "application logic must
not rely on provider-specific fields" means once it is a rule a test can
evaluate.

## Retries, and who owns them

`engineering-plan.md:1324` puts retries in the adapter.
`engineering-plan.md:1651` says "Keep retry decisions in application code, not
...
`engineering-plan.md:908` and has no counterpart here, which means today a
...
provider-opaque items at `engineering-plan.md:665` — store verbatim, never
...
`engineering-plan.md:663` defaults `ProviderReasoningItem.trust_level` to
...
Section 19's telemetry attributes (`engineering-plan.md:2198-2216`) omit the
...
Section 2.3's provider list at `engineering-plan.md:218-222` is controlling
...
(`engineering-plan.md:2622`) requires "the same contract suite against OpenAI,
Anthropic, and a chat_completions endpoint", while `engineering-plan.md:2366`
...
`engineering-plan.md:1284-1296` uses `FakeModelScript`, `ToolCallTurn` and
