...
This is not the `ModelCapabilities` of `engineering-plan.md:702`. Two
...
registry described at `engineering-plan.md:1391-1396`. The registry is
...
provider. Milestone 10 (`engineering-plan.md:3020-3027`) wants routing to move work
...
`engineering-plan.md:1391` calls a provider profile "a plugin the registry
loads and the user can override without editing core" and
`engineering-plan.md:1393-1396` says what it declares: an API mode, aliases
...
`engineering-plan.md:702` declares `ModelCapabilities` with eight fields and
...
```text
engineering-plan.md:702   here                     what changed
------------------------  -----------------------  ------------
...
Where the row for a field and `engineering-plan.md:702` conflict the plan
...
table to put them in. `runs.usage JSONB` at `engineering-plan.md:1783` is the
...
never disagree. The usage repository port named at `engineering-plan.md:913`
and never typed is:
...
`ModelTurn.provider_metadata` is declared at `engineering-plan.md:1308` as
`dict[str, Any]` and given exactly one rule at `engineering-plan.md:1310`:
...
| `response_id` | the response body | `engineering-plan.md:1353` requires the OpenAI adapter to capture it |
...
reads it, which is what `engineering-plan.md:1310`'s "application logic must
not rely on provider-specific fields" means once it is a rule a test can
evaluate.

## Retries, and who owns them

`engineering-plan.md:1356` puts retries in the adapter.
`engineering-plan.md:1683` says "Keep retry decisions in application code, not
...
`engineering-plan.md:940` and has no counterpart here, which means today a
...
provider-opaque items at `engineering-plan.md:697` — store verbatim, never
...
`engineering-plan.md:695` defaults `ProviderReasoningItem.trust_level` to
...
Section 19's telemetry attributes (`engineering-plan.md:2230-2248`) omit the
...
Section 2.3's provider list at `engineering-plan.md:250-254` is controlling
...
(`engineering-plan.md:2654`) requires "the same contract suite against OpenAI,
Anthropic, and a chat_completions endpoint", while `engineering-plan.md:2398`
...
`engineering-plan.md:1316-1328` uses `FakeModelScript`, `ToolCallTurn` and
