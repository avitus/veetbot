...
This is not the `ModelCapabilities` of `engineering-plan.md:630`. Two
...
registry described at `engineering-plan.md:1315-1322`. The registry is
...
Section 10 (`engineering-plan.md:1332`) requires a run to be pinned to one
provider. Milestone 10 (`engineering-plan.md:2950`) wants routing to move work
...
`engineering-plan.md:1315` calls a provider profile "a plugin the registry
loads and the user can override without editing core" and
`engineering-plan.md:1317-1320` says what it declares: an API mode, aliases
...
profile. `bootstrap-and-composition.md:374-375` places `models/policies.yaml`
...
the four things `bootstrap-and-composition.md:375` names it for and becomes
...
family of the secret scanner at `bootstrap-and-composition.md:1138-1179` is
...
`policy_version` at `policy-and-approvals.md:675` because it answers the
...
`engineering-plan.md:630` declares `ModelCapabilities` with eight fields and
...
```text
engineering-plan.md:630   here                     what changed
------------------------  -----------------------  ------------
...
Where the row for a field and `engineering-plan.md:630` conflict the plan
...
table to put them in. `runs.usage JSONB` at `engineering-plan.md:1707` is the
...
never disagree. The usage repository port named at `engineering-plan.md:837`
and never typed is:
...
`ModelTurn.provider_metadata` is declared at `engineering-plan.md:1232` as
`dict[str, Any]` and given exactly one rule at `engineering-plan.md:1234`:
...
reads it, which is what `engineering-plan.md:1234`'s "application logic must
...
`engineering-plan.md:864` and has no counterpart here, which means today a
...
provider-opaque items at `engineering-plan.md:621` — store verbatim, never
...
`engineering-plan.md:623` defaults `ProviderReasoningItem.trust_level` to
`TrustLevel.PLATFORM`. That is the highest trust tier in the system, and
`policy-and-approvals.md:870-899` maps trust tiers to policy restrictiveness,
...
Section 19's telemetry attributes (`engineering-plan.md:2154-2163`) omit the
...
Section 2.3's provider list at `engineering-plan.md:178-182` is controlling
...
(`engineering-plan.md:2578`) requires "the same contract suite against OpenAI,
Anthropic, and a chat_completions endpoint", while `engineering-plan.md:2326`
...
`engineering-plan.md:1243-1251` uses `FakeModelScript`, `ToolCallTurn` and
...
7. Is one file per provider profile right, given that
   `bootstrap-and-composition.md:374` describes a single `models/policies.yaml`
