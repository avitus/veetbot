...
This is not the `ModelCapabilities` of `engineering-plan.md:729`. Two
...
provider. Milestone 10 (`engineering-plan.md:3050-3057`) wants routing to move work
...
profile. `bootstrap-and-composition.md:386-387` places `models/policies.yaml`
...
the four things `bootstrap-and-composition.md:387` names it for and becomes
...
family of the secret scanner at `bootstrap-and-composition.md:1172-1213` is
...
`engineering-plan.md:729` declares `ModelCapabilities` with eight fields and
...
```text
engineering-plan.md:729   here                     what changed
------------------------  -----------------------  ------------
...
Where the row for a field and `engineering-plan.md:729` conflict the plan
...
never disagree. The usage repository port named at `engineering-plan.md:940`
and never typed is:
...
`engineering-plan.md:967` and has no counterpart here, which means today a
...
provider-opaque items at `engineering-plan.md:724` — store verbatim, never
...
7. Is one file per provider profile right, given that
   `bootstrap-and-composition.md:386` describes a single `models/policies.yaml`
