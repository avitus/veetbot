...
This is not the `ModelCapabilities` of `engineering-plan.md:700`. Two
...
registry described at `engineering-plan.md:1389-1394`. The registry is
...
Section 10 (`engineering-plan.md:1387`) requires a run to be pinned to one
provider. Milestone 10 (`engineering-plan.md:3018-3025`) wants routing to move work
...
`engineering-plan.md:1389` calls a provider profile "a plugin the registry
loads and the user can override without editing core" and
`engineering-plan.md:1391-1394` says what it declares: an API mode, aliases
...
profile. `bootstrap-and-composition.md:384-385` places `models/policies.yaml`
...
the four things `bootstrap-and-composition.md:385` names it for and becomes
...
family of the secret scanner at `bootstrap-and-composition.md:1170-1211` is
...
`engineering-plan.md:700` declares `ModelCapabilities` with eight fields and
...
```text
engineering-plan.md:700   here                     what changed
------------------------  -----------------------  ------------
...
Where the row for a field and `engineering-plan.md:700` conflict the plan
...
table to put them in. `runs.usage JSONB` at `engineering-plan.md:1781` is the
...
never disagree. The usage repository port named at `engineering-plan.md:911`
and never typed is:
...
`ModelTurn.provider_metadata` is declared at `engineering-plan.md:1306` as
`dict[str, Any]` and given exactly one rule at `engineering-plan.md:1308`:
...
| `response_id` | the response body | `engineering-plan.md:1351` requires the OpenAI adapter to capture it |
...
reads it, which is what `engineering-plan.md:1308`'s "application logic must
not rely on provider-specific fields" means once it is a rule a test can
evaluate.

## Retries, and who owns them

`engineering-plan.md:1354` puts retries in the adapter.
`engineering-plan.md:1681` says "Keep retry decisions in application code, not
...
`engineering-plan.md:938` and has no counterpart here, which means today a
...
provider-opaque items at `engineering-plan.md:695` — store verbatim, never
...
`engineering-plan.md:693` defaults `ProviderReasoningItem.trust_level` to
...
Section 19's telemetry attributes (`engineering-plan.md:2228-2246`) omit the
...
Section 2.3's provider list at `engineering-plan.md:248-252` is controlling
...
(`engineering-plan.md:2652`) requires "the same contract suite against OpenAI,
Anthropic, and a chat_completions endpoint", while `engineering-plan.md:2396`
...
`engineering-plan.md:1314-1326` uses `FakeModelScript`, `ToolCallTurn` and
...
7. Is one file per provider profile right, given that
   `bootstrap-and-composition.md:384` describes a single `models/policies.yaml`
