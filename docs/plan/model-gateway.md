...
This is not the `ModelCapabilities` of `engineering-plan.md:616`. Two
...
registry described at `engineering-plan.md:1305-1312`. The registry is
...
Section 10 (`engineering-plan.md:1322`) requires a run to be pinned to one
provider. Milestone 10 (`engineering-plan.md:2940`) wants routing to move work
...
`engineering-plan.md:1305` calls a provider profile "a plugin the registry
loads and the user can override without editing core" and
`engineering-plan.md:1307-1310` says what it declares: an API mode, aliases
...
family of the secret scanner at `bootstrap-and-composition.md:1118-1155` is
...
`engineering-plan.md:616` declares `ModelCapabilities` with eight fields and
...
```text
engineering-plan.md:616   here                     what changed
------------------------  -----------------------  ------------
...
Where the row for a field and `engineering-plan.md:616` conflict the plan
...
table to put them in. `runs.usage JSONB` at `engineering-plan.md:1697` is the
...
never disagree. The usage repository port named at `engineering-plan.md:827`
and never typed is:
...
`ModelTurn.provider_metadata` is declared at `engineering-plan.md:1222` as
`dict[str, Any]` and given exactly one rule at `engineering-plan.md:1224`:
...
| `response_id` | the response body | `engineering-plan.md:1267` requires the OpenAI adapter to capture it |
...
reads it, which is what `engineering-plan.md:1224`'s "application logic must
not rely on provider-specific fields" means once it is a rule a test can
evaluate.

## Retries, and who owns them

`engineering-plan.md:1270` puts retries in the adapter.
`engineering-plan.md:1597` says "Keep retry decisions in application code, not
...
`engineering-plan.md:854` and has no counterpart here, which means today a
...
provider-opaque items at `engineering-plan.md:611` — store verbatim, never
...
`engineering-plan.md:609` defaults `ProviderReasoningItem.trust_level` to
...
Section 19's telemetry attributes (`engineering-plan.md:2144-2153`) omit the
...
(`engineering-plan.md:2568`) requires "the same contract suite against OpenAI,
Anthropic, and a chat_completions endpoint", while `engineering-plan.md:2312`
...
`engineering-plan.md:1233-1241` uses `FakeModelScript`, `ToolCallTurn` and
