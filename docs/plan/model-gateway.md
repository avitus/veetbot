...
This is not the `ModelCapabilities` of `engineering-plan.md:658`. Two
...
`engineering-plan.md:658` declares `ModelCapabilities` with eight fields and
...
```text
engineering-plan.md:658   here                     what changed
------------------------  -----------------------  ------------
...
Where the row for a field and `engineering-plan.md:658` conflict the plan
...
| `response_id` | the response body | `engineering-plan.md:1309` requires the OpenAI adapter to capture it |
...
`engineering-plan.md:1312` puts retries in the adapter.
`engineering-plan.md:1639` says "Keep retry decisions in application code, not
...
`engineering-plan.md:651` defaults `ProviderReasoningItem.trust_level` to
...
`engineering-plan.md:1272-1284` uses `FakeModelScript`, `ToolCallTurn` and
