...
- The `balanced` default selects OpenAI GPT-6 Astra and `flagship` selects
  Claude Fable 5.1. Explicit `astra` and `fable` policies are also available;
...
- Passed all twenty notification and device gates, all 247 cumulative gates,
  ...
  unsigned Release builds, hosted CI, and direct delivery to `dev`. Code review
  is reserved for the final merge into `main`. Physical
...
- Left the verdict and the gate census alone. Milestone 10 is still
  ...
  and no document was added, so the census stays a…10280 tokens truncated… container
