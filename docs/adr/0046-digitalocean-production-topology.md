...
- **Status:** Accepted; decision 2 and the Docker-group consequence
  superseded by ADR-0067 (a credential-free execution service, not the worker,
  owns the Docker adapter and the `docker` group membership); decision 4
  superseded by ADR-0048 (Nginx, not Caddy, terminates TLS on the host);
  decision 7 remains the current production posture until Milestone 15
  (ADR-0065, authorized by ADR-0061, not yet implemented) lands the controls
  it declined
- **Date:** 2026-08-07; amended 2026-08-21

## Amendments

- 2026-08-10, ADR-0048: Nginx, not Caddy, is the reverse proxy and TLS
  terminator on this host; the Caddy wording in decision 4 is historical.
...
- 2026-08-21, ADR-0067: the host-native single-Droplet decision remains, but
  decision 2's three-unit count and worker-only `docker` membership, the
  worker-owned gVisor adapter described in the context below, and the
  Docker-group consequence are superseded. A separate
  `veetbot-execution.service` under the `veetbot-exec` identity owns the Docker
  adapter and the `docker` group membership; no application unit (API, worker,
  or maintenance) joins that group or sees the socket. Decision 3's `runsc`
  requirement stands under the new owner.
...
2. Three systemd units run the API, durable worker, and maintenance worker. Only
   the worker joins the `docker` group. *(Superseded by ADR-0067.)*
...
- Docker-group membership is root-equivalent host authority. It is confined to
  ...
  service. *(Superseded by ADR-0067: the execution service holds this
  authority; no worker does.)*
