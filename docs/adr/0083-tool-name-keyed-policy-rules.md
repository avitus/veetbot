# ADR-0083: Tool-name-keyed policy rules and declared human confirmation

- Status: Accepted
- Date: 2026-09-01
- Related: engineering plan Sections 9.2, 9.3, and 11.2; ADR-0017, ADR-0081
- Detailed design: `docs/plan/policy-and-approvals.md`,
  `docs/plan/device-channel-and-sms.md`

## Context

ADR-0081 decided that `device.sms.send` classifies `ALLOW`, because iOS makes
...
4. **The real trust model is the compose sheet, and the ADR says so.** The
   ...
   send without a tap (ADR-0081). Policy's origin escalation is defense in
...
- **Model the compose-sheet tap as standing authorization.** Rejected: it keeps
  ...
  contradicts ADR-0081's plain statement that policy classifies the tool
